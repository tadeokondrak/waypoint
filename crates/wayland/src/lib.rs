...
pub trait Object<P: Protocols>: Debug + Copy {
    const INTERFACE: P::Interface;
...
pub trait Protocols {
    type Interface: Debug + Copy;
    type Request<'a>: Debug;
    type Event<'a>: Debug;
    fn interface_name(interface: Self::Interface) -> &'static str;
    fn interface_version(interface: Self::Interface) -> u32;
    fn marshal_event(request: Self::Event<'_>, conn: &mut Connection);
    fn marshal_request(request: Self::Request<'_>, conn: &mut Connection);
    fn unmarshal_event(
        interface: Self::Interface,
        msg: Message<'_>,
    ) -> Option<Self::Event<'static>>;
    fn unmarshal_request(
        interface: Self::Interface,
        msg: Message<'_>,
    ) -> Option<Self::Request<'static>>;
}
