...
impl<'a> GenContext<'a> {
    fn gen(&self) -> TokenStream {
        ...
        quote! {
            ...
            impl wayland::Protocols for Protocols {
                ...
                fn interface_version(interface: Interface) -> u32 {
                    interface.version()
                }
                fn marshal_event(request: Event<'_>, conn: &mut Connection) {
                    Event::marshal(request, conn)
                }
                fn marshal_request(request: Request<'_>, conn: &mut Connection) {
                    Request::marshal(request, conn)
                }
                fn unmarshal_event(interface: Interface, msg: Message<'_>) -> Option<Event<'static>> {
                    Event::unmarshal(interface, msg)
                }
                fn unmarshal_request(interface: Interface, msg: Message<'_>) -> Option<Request<'static>> {
                    Request::unmarshal(interface, msg)
                }
            }
    ...
    fn gen_interface(&self, interface: &Interface) -> TokenStream {
        ...
        let interface_struct_object_impl = quote! {
            impl Object<Protocols> for #type_name {
