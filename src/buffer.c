...
static struct waypoint_buffer *waypoint_buffer_create(
    struct wl_shm *wl_shm, struct wl_list *buffers, int width, int height)
{
    struct waypoint_buffer *buffer = calloc(1, sizeof *buffer);
...
struct waypoint_buffer *waypoint_buffer_get(struct wl_shm *wl_shm,
    struct wl_list *buffers, int width, int height)
{
    bool found = false;
    ...
    wl_list_for_each_safe(buffer, tmp, buffers, link) {
        ...
        if (buffer->width != width || buffer->height != height) {
            waypoint_buffer_destroy(buffer);
            continue;
        }
        cairo_destroy(buffer->cairo);
        buffer->cairo = cairo_create(buffer->cairo_surface);
    ...
    if (!found)
        buffer = waypoint_buffer_create(wl_shm, buffers, width, height);
